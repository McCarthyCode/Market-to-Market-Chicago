...
(b+1)+")").addClass("active");break;case 40:b=-1===b?0:(b+g-1)%g,$("#neighborhoodAutocomplete li").removeClass("active"),$("#neighborhoodAutocomplete li:nth-child("+(b+1)+")").addClass("active")}});v.on("click","ul li",function(){n.val($(this).data("id"));e.val($(this).text());$("#neighborhoodAutocomplete ul").remove()});e.focusout(function(){setTimeout(function(){$("#neighborhoodAutocomplete ul").remove()},100)});var c=-1,h=0,w=$("#locationAutocomplete");f.on("input",function(a){$("#locationAutocomplete ul").remove();
m.val(0);a=$(this).val();""!==a&&$.get("/locations/autocomplete",{q:a},function(a){w.append(a);c=-1;h=$("#locationAutocomplete li").length})});f.keydown(function(a){switch(a.keyCode){case 13:a.preventDefault();a=$("#locationAutocomplete li.active");1===$("#locationAutocomplete li").length&&(a=$("#locationAutocomplete li:first-child"));m.val(a.data("id"));f.val(a.text());$("#locationAutocomplete ul").remove();k();break;case 38:c=-1===c?h-1:(c+1)%h;$("#locationAutocomplete li").removeClass("active");
$("#locationAutocomplete li:nth-child("+(c+1)+")").addClass("active");break;case 40:c=-1===c?0:(c+h-1)%h,$("#locationAutocomplete li").removeClass("active"),$("#locationAutocomplete li:nth-child("+(c+1)+")").addClass("active")}});w.on("click","ul li",function(){m.val($(this).data("id"));f.val($(this).text());$("#locationAutocomplete ul").remove();k()});f.focusout(function(){setTimeout(function(){$("#locationAutocomplete ul").remove()},100)})});
